  ...
  describe "ngo custom validators" do
    ...
    test "changeset/2 fails if pre-registration-start-date is equal to or greater than launch date" do
      ngo = NGO.changeset(
        ...
        %{
          @create_attrs |
          pre_registration_start_date: Timex.shift(@create_attrs.pre_registration_start_date, days: 11),
    ...
    test "update_changeset/2 fails if admin edits pre_registeration_start after if it's in the past." do
      ngo = build(:ngo)
      updated_ngo = NGO.update_changeset(
        ngo,
        %{
          pre_registration_start_date: Timex.shift(ngo.pre_registration_start_date, minutes: 30),
          open_registration: false
        }
      )
