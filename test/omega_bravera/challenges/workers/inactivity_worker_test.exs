    ...
    test "updates inactive challenges" do
      ...
      still_active =
        Repo.all(
          from(c in NGOChal,
            where:
              c.participant_notified_of_inactivity == false and
                c.donor_notified_of_inactivity == false
          )
        )

      participant_notified =
        Repo.all(from(c in NGOChal, where: c.participant_notified_of_inactivity == true))
