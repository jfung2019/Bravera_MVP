defmodule OmegaBraveraWeb.UserChannelTest do
  ...
  describe "joined user channel" do
    ...
    test "can send out a message and get a broadcast of the newly created message", %{socket: socket, joined_partner: %{id: partner_id}} do
      message = "Hello World!"
      push(socket, "create_message", %{"message_params" => %{"group_id" => partner_id, "message" => message, "meta_data" => %{}}})
      assert_broadcast "new_message", %{message: %OmegaBravera.Groups.ChatMessage{message: ^message}}
      assert_push "new_message", %{message: %OmegaBravera.Groups.ChatMessage{message: ^message}}
    end

    test "can send back reply if bad data was sent", %{socket: socket, joined_partner: %{id: partner_id}} do
      ref = push(socket, "create_message", %{"message_params" => %{"group_id" => partner_id, "message" => nil, "meta_data" => %{}}})
