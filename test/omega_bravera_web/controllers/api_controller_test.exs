defmodule OmegaBraveraWeb.ApiControllerTest do
  ...
  test "a user with valid offer token can get a presigned URL and cdn URL", %{conn: conn} do
  ...
  test "a user with valid partner token can get a presigned URL and cdn URL", %{conn: conn} do
    ...
    assert %{
             "uploadURL" => "https://upload.com/partner_images/1/" <> filename,
             "fileURL" => "https://cdn.com/partner_images/1/" <> filename,
             "originalFilename" => ^original_filename
           } = json_response(conn, 200)
