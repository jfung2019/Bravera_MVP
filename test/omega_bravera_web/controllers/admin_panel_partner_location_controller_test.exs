defmodule OmegaBraveraWeb.AdminPanelPartnerLocationControllerTest do
  ...
  setup %{conn: conn} do
    with {:ok, admin_user} <-
         ...
         do: {:ok, conn: Plug.Conn.put_req_header(conn, "authorization", "bearer: " <> token), partner: Fixtures.partner_fixture()}
  end

  test "can create partner location", %{conn: conn, partner: partner} do
    conn = post(conn, Routes.admin_panel_partner_admin_panel_partner_location_path(conn, :create, partner), %{partner_location: %{address: "123", longitude: "123.4", latitude: "456.7"}})
  ...
  describe "partner location created" do
    ...
    test "can edit partner location form", %{conn: conn, partner: partner, location: location} do
      conn = get(conn, Routes.admin_panel_partner_admin_panel_partner_location_path(conn, :edit, partner, location))
      assert html_response(conn, 200)
    end

    test "can edit partner location", %{conn: conn, partner: partner, location: location} do
      conn = put(conn, Routes.admin_panel_partner_admin_panel_partner_location_path(conn, :update, partner, location), %{partner_location: %{address: "456", longitude: "123.5", latitude: "456.8"}})
    ...
    test "can delete partner location", %{conn: conn, partner: partner, location: location} do
      conn = delete(conn, Routes.admin_panel_partner_admin_panel_partner_location_path(conn, :delete, partner, location), %{partner_location: %{address: "456", longitude: "123.5", latitude: "456.8"}})
