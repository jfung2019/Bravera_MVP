window.addEventListener('load', function() {
  ...
  document.addEventListener("DOMContentLoaded",   function(event) {
    ...
    // Paid Offers form
    var offer_payment_form = document.getElementsByClassName("offer-payment-form");

    if(offer_payment_form) {  
      Array.from(offer_payment_form).forEach(function(element) {
        element.addEventListener('submit', function(event) {
          ...
          stripe.createToken(card).then(function(result) {
            if (result.error) {
              ...
              stripeTokenHandler(element, result.token)
            }
          });
        });
      });
