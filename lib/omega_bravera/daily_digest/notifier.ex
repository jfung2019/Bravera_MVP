defmodule OmegaBravera.DailyDigest.Notifier do
  ...
  @challenge_attachment_types [:new_challenges, :challenges_new_users, :challenges_milestones, :challenges_completed]
  ...
  def signups_digest_email(params) do
    ...
    Enum.reduce(@attachment_types, email, fn(type, email) -> add_csv_attachment(email, params, type) end)
  end

  defp text(%{signups: s, new_challenges: nc, challenges_new_users: cnu, challenges_milestones: cwm, challenges_completed: cc, new_donors: dn}) do
  ...
  defp add_csv_attachment(email, params, type) do
    ...
    if length(list) > 0 do
      Email.add_attachment(email, %{content: to_csv(list, type), filename: "#{type}_#{yesterday}.csv"})
    else
      email
    end
  end
  ...
  defp to_csv(list, type) do
    list
    |> csv_encode(type)
    |> Enum.take(length(list) + 1) #forcing the stream to be processed
    |> Enum.join("")
    |> Base.encode64()
  end
