defmodule OmegaBravera.Activity.Processor do
  ...
  alias OmegaBravera.{TaskSupervisor, Repo, Accounts, Activity.Activities, Offers.OfferActivitiesIngestion, Challenges.ActivitiesIngestion}
  ...
  def process_activity(%Strava.Activity{} = strava_activity, %{"owner_id" => athlete_id} = params) do
    ...
    case Activities.create_activity(strava_activity, user) do
      {:ok, activity} ->
        ...
        Task.Supervisor.start_child(TaskSupervisor, OfferActivitiesIngestion, :start, [activity, params], [restart: :temporary])
        Task.Supervisor.start_child(TaskSupervisor, ActivitiesIngestion, :start, [activity, params], [restart: :temporary])
