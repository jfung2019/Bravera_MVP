defmodule OmegaBravera.Challenges.Notifier do
  ...
  def activity_completed_email(%NGOChal{} = challenge, %Activity{} = activity) do
    ...
    |> Email.add_substitution("-challengeURL-", "#{Application.get_env(:omega_bravera, :app_base_url)}/#{challenge.ngo.slug}/#{challenge.slug}")
    |> Email.put_from("admin@bravera.co", "Bravera")
    |> Email.add_bcc("admin@bravera.co")
    |> Email.add_to(challenge.user.email)
  end
  ...
  def donor_milestone_email(%Donation{} = donation) do
    ...
    |> Email.add_substitution("-challengeURL-", "#{Application.get_env(:omega_bravera, :app_base_url)}/#{donation.ngo.slug}/#{donation.ngo_chal.slug}")
    |> Email.put_from("admin@bravera.co", "Bravera")
    |> Email.add_bcc("admin@bravera.co")
